...
class Solution {
public:   
    vector<vector<int>> twoSum(vector<int>::iterator begin, vector<int>::iterator end,int target) {
    ...
    vector<vector<int> > threeSum(vector<int>::iterator begin, vector<int>::iterator end,int target) {
    	vector<vector<int>> result;
    	if(end-begin<3)
    		return result;
    	vector<int>::iterator iter1=begin,iter2=iter1;
    	while(iter1<end-2)
    	{
    		vector<vector<int>> tempResult;
    		int target0=target-*iter1;
    		tempResult=twoSum(++iter2,end,target0);
    		if(!tempResult.empty())
    		{
    		...
    		int key=*iter1;
    		while(iter1<end-2&&*iter1==key)//循环内循环注意同样要保持外循环的条件，否则易出错 
    			iter1++;
    		iter2=iter1;
    	}
    	return result;   
    ...
    vector<vector<int> > fourSum(vector<int> &num, int target) {
        vector<vector<int>> result;
    	if(num.end()-num.begin()<4)
    		return result;
    	sort(num.begin(),num.end());
    	vector<int>::iterator iter1=num.begin(),iter2=iter1;
    	while(iter1<num.end()-3)
    	{
    		vector<vector<int>> tempResult;
    		int target0=target-*iter1;
    		tempResult=threeSum(++iter2,num.end(),target0);
    		if(!tempResult.empty())
    		{
    			for(vector<vector<int>>::iterator iter=tempResult.begin();iter!=tempResult.end();iter++)
    			{
    				...
    				temp.push_back((*iter)[0]);
    				temp.push_back((*iter)[1]);
    				temp.push_back((*iter)[2]);
    				result.push_back(temp);
    			}
    		}
    		int key=*iter1;
    		while(iter1<num.end()-3&&*iter1==key)//循环内循环注意同样要保持外循环的条件，否则易出错 
    			iter1++;
    		iter2=iter1;
    	}
    	return result; 
